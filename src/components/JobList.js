...
const JobList = ({ title, link, data }) => {
  const handleDelete = async (jobId) => {
    const thisJob = await data?.find(
      (job) => parseInt(job.id) === parseInt(jobId)
    );

    if (thisJob.colaborateur.length > 0) {
      alert("Vous ne pouvez pas suprimer ce élément !");
    } else
      try {
        await axios.delete(`${BASEURL}poste/${jobId}`);
