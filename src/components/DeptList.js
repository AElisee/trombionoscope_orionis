...
const DeptList = ({ title, link, data }) => {
  const handleDelete = async (deptId) => {
    const thisdept = await data?.find(
      (dept) => parseInt(dept.id) === parseInt(deptId)
    );
    if (thisdept.service.length > 0) {
      alert("Vous ne pouvez pas suprimer ce élément !");
    } else
      try {
        await axios.delete(`${BASEURL}departement/${deptId}`);
  ...
  const columns = [
    {
      field: "poste",
      headerName: <span className="bold-header">DEPARTEMENT</span>,
      width: 400,
